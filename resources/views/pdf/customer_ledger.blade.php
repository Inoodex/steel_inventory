    ...
    <table class="header-table" cellpadding="0" cellspacing="0">
        <tr>
            <td style="width: 50%; vertical-align: middle;">
                <div class="party-title">{{ $customer->name }}</div>
                <div class="party-meta">
                    @if($customer->company)<strong>{{ $customer->company }}</strong><br>@endif
                    <span>Customer ID: #CUST-{{ str_pad($customer->id, 4, '0', STR_PAD_LEFT) }}</span>
                </div>
            </td>
        ...
        <table cellpadding="0" cellspacing="0">
            <tr>
                ...
                <td style="width: 25%; text-align: right;" class="party-meta">
                    @if($customer->bin_number)<strong>BIN:</strong> {{ $customer->bin_number }}<br>@endif
                    @if($customer->tin_number)<strong>TIN:</strong> {{ $customer->tin_number }}<br>@endif
                    <strong>Status:</strong> {{ in_array($customer->status, ['1', 'active', 1]) ? 'Active' : 'Inactive' }}
                </td>
            </tr>
        </table>
    </div>
