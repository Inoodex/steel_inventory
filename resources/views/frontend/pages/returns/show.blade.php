...
<div class="content container-fluid">
    ...
    @if(!$return)
        <div class="alert alert-warning rounded-3">
            <i class="fe fe-alert-triangle me-2"></i> Return record not found.
            <a href="{{ route('returns.index') }}" class="alert-link ms-2">Back to Returns</a>
        </div>
    @else
        <!-- Page Header -->
        ...
        @php
            $statusConfig = [
                ...
                'approved' => ['class' => 'alert-success text-dark', 'icon' => 'fe-check-circle', 'msg' => 'Return approved. Inventory stock restocked and invoice balance adjusted.'],
                'completed' => ['class' => 'alert-success text-dark', 'icon' => 'fe-check-circle', 'msg' => 'Return process completed and inventory stock updated.'],
                        ...
                        <tbody class="border-top-0">
                            @foreach($return->items as $item)
                                <tr>
                                    ...
                                    <td>
                                        @php
                                            $productName = $item->product->coil_number
                                                ?? $item->product->name
                                                ?? ($item->salesItem ? 'Coil (Sale Item #' . $item->salesItem->id . ')' : 'N/A');
                                        @endphp
        ...
        <!-- Actions Bar Card (Only for Pending Returns) -->
        @if($return->isPending())
            <div class="card border-0 shadow-sm rounded-3 mb-4">
                <div class="card-body p-4 d-flex justify-content-between align-items-center flex-wrap gap-3">
                    <div>
                        <h6 class="fw-bold text-dark mb-1"><i class="fe fe-settings me-2 text-primary"></i>Approval Actions</h6>
                        <span class="text-muted small">Approving this return will immediately restock items into inventory and adjust the invoice balance.</span>
                    </div>
                    <div class="d-flex gap-2">
                        <form method="POST" action="{{ route('returns.approve', $return->id) }}" class="d-inline" onsubmit="return confirm('Approve return? Stock will be updated and invoice adjusted immediately.')">
                            @csrf
                            @method('PATCH')
                            <button type="submit" class="btn btn-success px-4 py-2 rounded-3 d-flex align-items-center gap-2">
                                <i class="fe fe-check-circle"></i>
                                <span>Approve & Restock</span>
                            </button>
                        </form>

                        <button type="button" class="btn btn-outline-danger px-4 py-2 rounded-3 d-flex align-items-center gap-2" data-bs-toggle="modal" data-bs-target="#rejectModal">
