...
<div class="content container-fluid">
    ...
    <div class="page-header mb-4">
        <div class="content-page-header d-flex flex-wrap justify-content-between align-items-center gap-3">
            <div>
                <h4 class="card-title fw-bold text-dark mb-1">Edit Vendor Profile</h4>
                <p class="text-muted small mb-0">Update contact details, email address, or status for {{ $vendor->name ?? $customer->name }}</p>
            </div>
            <div>
    ...
    <div class="card border-0 shadow-sm rounded-3">
        <div class="card-body p-4">
            ...
            <form method="POST" action="{{ route('vendors.update', $vData->id) }}">
                @csrf
                @method('PUT')
                ...
                <div class="row g-3 mb-4">
                    ...
                    <div class="col-md-6 col-lg-4">
                        ...
                        <input type="text" class="form-control @error('name') is-invalid @enderror" name="name" value="{{ old('name', $vData->name) }}" required>
                        @error('name')
                    ...
                    <div class="col-md-6 col-lg-4">
                        ...
                        <input type="tel" class="form-control @error('phone') is-invalid @enderror" name="phone" value="{{ old('phone', $vData->phone) }}" pattern="[0-9]{11}" maxlength="11" required>
                        @error('phone')
                            <div class="text-danger fs-7 mt-1">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="col-md-6 col-lg-4">
                        ...
                        <input type="email" class="form-control @error('email') is-invalid @enderror" name="email" value="{{ old('email', $vData->email) }}">
                        @error('email')
                            <div class="text-danger fs-7 mt-1">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="col-md-6 col-lg-4">
                        ...
                        @php
                            $currStatus = in_array($vData->status, ['active', '1', 1]) ? 'active' : 'inactive';
                        @endphp
                    ...
                    <div class="col-12">
                        ...
                        <textarea class="form-control @error('address') is-invalid @enderror" name="address" rows="3" required>{{ old('address', $vData->address) }}</textarea>
                        @error('address')
                            <div class="text-danger fs-7 mt-1">{{ $message }}</div>
                        @enderror
                    </div>
                ...
                <div class="d-flex align-items-center justify-content-end gap-2 pt-3 border-top border-light">
                    ...
                    <button type="submit" class="btn btn-primary px-4 py-2 rounded-3 shadow-sm">Update Vendor</button>
                </div>
