...
<div class="content container-fluid">
    ...
    <div class="page-header mb-4">
        <div class="content-page-header d-flex flex-wrap justify-content-between align-items-center gap-3">
            <div>
                <h4 class="card-title fw-bold text-dark mb-1">Vendor Account Statement &amp; Ledger</h4>
                <p class="text-muted small mb-0">Chronological procurement bills, disbursements, and running supplier payable balance</p>
            </div>
            <div class="d-flex align-items-center gap-2">
                <a href="{{ route('vendors.ledger.pdf', array_merge(['id' => $vendor->id], request()->all())) }}" target="_blank" class="btn btn-primary px-3 py-2 rounded-3 d-inline-flex align-items-center gap-2 shadow-sm">
                    <i class="fe fe-download"></i>
                    <span>Download Statement PDF</span>
                </a>
                <a href="{{ route('vendors.show', $vendor->id) }}" class="btn btn-outline-secondary px-3 py-2 rounded-3 d-inline-flex align-items-center gap-2">
    ...
    <!-- Vendor Bio Info & Filter Bar -->
    <div class="card border-0 shadow-sm rounded-3 mb-4">
        <div class="card-body p-4">
            <div class="row align-items-center g-3">
                <div class="col-lg-5 col-12">
                    <div class="d-flex align-items-center gap-3">
                        <div class="avatar avatar-xl rounded-circle bg-warning-light text-warning fw-bold fs-4 d-flex align-items-center justify-content-center flex-shrink-0">
                            {{ strtoupper(substr($vendor->name, 0, 2)) }}
                        </div>
                        <div>
                            <h5 class="fw-bold text-dark mb-1">{{ $vendor->name }}</h5>
                            <p class="text-muted small mb-0">
                                @if($vendor->company)<span class="fw-semibold text-dark">{{ $vendor->company }}</span> &bull; @endif
                                <span>{{ $vendor->phone }}</span>
                                @if($vendor->email) &bull; <span>{{ $vendor->email }}</span>@endif
                            </p>
                            <small class="text-secondary">{{ $vendor->address }}</small>
                        </div>
                    </div>
                </div>

                <div class="col-lg-7 col-12">
                    <form method="GET" action="{{ route('vendors.ledger', $vendor->id) }}" class="row g-2 align-items-end justify-content-lg-end">
                        ...
                        <div class="col-sm-4 col-12 d-flex gap-2">
                            ...
                            <a href="{{ route('vendors.ledger', $vendor->id) }}" class="btn btn-sm btn-outline-secondary rounded-3 py-2 px-3" title="Reset Filters">
        ...
        <div class="col-xl-3 col-sm-6 col-12">
            <div class="card border-0 shadow-sm rounded-3 h-100 mb-0">
                <div class="card-body p-3 d-flex align-items-center">
                    ...
                    <div>
                        <span class="text-muted small d-block">Total Purchases (Credit)</span>
                        <h5 class="mb-0 fw-bold text-primary">৳{{ number_format($totalCredit, 2) }}</h5>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="col-xl-3 col-sm-6 col-12">
            <div class="card border-0 shadow-sm rounded-3 h-100 mb-0">
                <div class="card-body p-3 d-flex align-items-center">
                    ...
                    <div>
                        <span class="text-muted small d-block">Total Disbursed (Debit)</span>
                        <h5 class="mb-0 fw-bold text-success">৳{{ number_format($totalDebit, 2) }}</h5>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="col-xl-3 col-sm-6 col-12">
            <div class="card border-0 shadow-sm rounded-3 h-100 mb-0 {{ $closingBalance > 0 ? 'bg-danger-subtle border border-danger-subtle' : 'bg-success-subtle border border-success-subtle' }}">
                <div class="card-body p-3 d-flex align-items-center">
                    ...
                    <div>
                        <span class="{{ $closingBalance > 0 ? 'text-danger' : 'text-success' }} fw-semibold small d-block">Net Outstanding Payable</span>
                ...
                <table class="table table-hover table-custom align-middle mb-0">
                    <thead class="bg-light text-secondary fs-7 text-uppercase">
                        <tr>
                            ...
                            <th>Description</th>
                            <th class="text-end" style="width: 140px;">Paid / Debit (৳)</th>
                            <th class="text-end" style="width: 140px;">Bill / Credit (৳)</th>
                            <th class="pe-4 text-end" style="width: 160px;">Payable Balance</th>
                        </tr>
                    </thead>
                    <tbody class="border-top-0">
                        ...
                        <tr class="bg-light-subtle">
                            ...
                            <td class="text-muted small">Initial / Brought Forward Payable Balance</td>
                        ...
                        @forelse($ledgerRows as $index => $row)
                            <tr>
                                ...
                                <td class="text-end fw-semibold {{ $row['debit'] > 0 ? 'text-success' : 'text-muted' }}">
                                    {{ $row['debit'] > 0 ? '৳' . number_format($row['debit'], 2) : '-' }}
                                </td>
                                <td class="text-end fw-semibold {{ $row['credit'] > 0 ? 'text-primary' : 'text-muted' }}">
                    ...
                    <tfoot class="bg-light border-top">
                        <tr class="fw-bold text-dark">
                            <td colspan="4" class="ps-4 text-end text-uppercase fs-7">Totals &amp; Net Closing Payable:</td>
                            <td class="text-end text-success">৳{{ number_format($totalDebit, 2) }}</td>
                            <td class="text-end text-primary">৳{{ number_format($totalCredit, 2) }}</td>
